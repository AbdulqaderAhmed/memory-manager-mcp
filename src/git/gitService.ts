...
async function getRecentCommits(cwd: string, count = 5): Promise<GitCommitInfo[]> {
  try {
 ...
 */
export async function getGitInfo(cwd: string, options?: { includeCommits?: boolean }): Promise<GitInfo> {
