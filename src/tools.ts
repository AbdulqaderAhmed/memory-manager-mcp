...
const workspaceArg = z
  ...
  .optional()
  .describe(
    "Workspace directory of the project. Defaults to the server working directory.",
  );
...
const agentIdArg = z
  ...
  .optional()
  .describe(
    'Identifier of the calling agent/client, e.g. "cursor", "vscode", "claude-cli".',
  );
...
export function registerTools(server: McpServer, service: MemoryService): void {
    ...
    "initialize_project_context",
    {
      ...
      inputSchema: {
        ...
        focus: z
          .string()
          .optional()
          .describe("Optional focus topic to bias memory selection."),
      },
    },
    async ({ workspacePath, agentId, focus }) => {
      try {
        const result = await service.initializeContext(workspacePath, {
          focus,
        });
        if (agentId) {
  ...
  server.registerTool(
    ...
    async ({ workspacePath }) => {
      try {
        ...
        return text(
          context ?? { projectId: project.id, note: "No context stored yet." },
        );
      } catch (err) {
        return fail(err);
      }
    ...
    "save_memory",
    {
      ...
      inputSchema: {
        ...
        importance: z
          ...
          .max(1)
          .optional()
          .describe("0..1, default 0.5."),
        confidence: z
          .number()
          .min(0)
          .max(1)
          .optional()
          .describe("0..1, default 0.7."),
        source: z
          .string()
          .optional()
          .describe('Where this came from, e.g. "user", "debugging".'),
        agentId: agentIdArg,
        ...
        id: z
          .string()
          .optional()
          .describe("Provide to update an existing memory."),
      },
    },
    async (args) => {
      try {
  ...
  server.registerTool(
    ...
    async ({ workspacePath, memoryId }) => {
      try {
        const { project } = await service.detectAndRegister(workspacePath);
        const memory = await service.memoryManager.getMemory(
          project.id,
          memoryId,
        );
    ...
    "search_memory",
    {
      ...
      inputSchema: {
        ...
        types: z
          ...
          .optional()
          .describe("Restrict to memory types."),
    ...
    async ({ workspacePath }) => {
      try {
        ...
        const open = all.filter((t) =>
          ["active", "in_progress", "blocked"].includes(t.status),
        );
  ...
  server.registerTool(
    ...
    async (args) => {
      try {
        ...
        if (!args.title)
          return fail(new Error("title is required when creating a task"));
  ...
  server.registerTool(
    "record_decision",
    {
      ...
      inputSchema: {
        ...
        content: z
          .string()
          .describe('The decision, e.g. "Use RBAC for employee permissions."'),
    ...
    "get_decisions",
    {
      ...
      inputSchema: {
        ...
        activeOnly: z
          .boolean()
          .optional()
          .describe("Exclude superseded decisions."),
      },
    },
    async ({ workspacePath, activeOnly }) => {
      try {
        const { project } = await service.detectAndRegister(workspacePath);
        const decisions = await service.memoryManager.getDecisions(
          project.id,
          activeOnly ?? true,
        );
  ...
  server.registerTool(
    "create_handoff",
    {
      ...
      inputSchema: {
        ...
        nextAction: z
          .string()
          .describe("Recommended next action for the next agent."),
    ...
    "get_latest_handoff",
    {
      ...
      inputSchema: {
        ...
        history: z
          .boolean()
          .optional()
          .describe("Also include recent handoff history."),
      },
    },
    async ({ workspacePath, history }) => {
      try {
        const { project } = await service.detectAndRegister(workspacePath);
        const latest = await service.handoffManager.getLatestHandoff(
          project.id,
        );
        if (!latest)
          return text({
            found: false,
            note: "No handoff exists for this project yet.",
          });
        if (history) {
    ...
    "start_session",
    {
      ...
      inputSchema: {
        ...
        agentName: z
          .string()
          .optional()
          .describe('Human-friendly client name, e.g. "Cursor".'),
      },
    },
    async ({ workspacePath, agentId, agentName }) => {
      try {
        const { detection, project } =
          await service.detectAndRegister(workspacePath);
        const git =
          detection.git ??
          (await getGitInfo(detection.workspacePath, {
  ...
  server.registerTool(
    ...
    async ({ confirm, confirmPhrase }) => {
      try {
        if (
          !confirm ||
          confirmPhrase.trim().toLowerCase() !== "delete everything"
        ) {
          return text({
            cleared: false,
