...
export function getProjectDir(projectId: string, root: string = getMemoryRoot()): string {
  return path.join(getProjectsDir(root), sanitizeSegment(projectId));
}

export function getProjectFile(projectId: string, root: string = getMemoryRoot()): string {
  return path.join(getProjectDir(projectId, root), 'project.json');
}

export function getContextFile(projectId: string, root: string = getMemoryRoot()): string {
  return path.join(getProjectDir(projectId, root), 'context.json');
}

export function getTasksFile(projectId: string, root: string = getMemoryRoot()): string {
  return path.join(getProjectDir(projectId, root), 'tasks.json');
}

export function getDecisionsFile(projectId: string, root: string = getMemoryRoot()): string {
  return path.join(getProjectDir(projectId, root), 'decisions.json');
}

export function getMemoriesFile(projectId: string, root: string = getMemoryRoot()): string {
  return path.join(getProjectDir(projectId, root), 'memories.jsonl');
}

export function getSessionsDir(projectId: string, root: string = getMemoryRoot()): string {
...
export function getSessionFile(
  ...
  return path.join(getSessionsDir(projectId, root), `${sanitizeSegment(sessionId)}.json`);
}

export function getHandoffsDir(projectId: string, root: string = getMemoryRoot()): string {
  return path.join(getProjectDir(projectId, root), 'handoffs');
}

export function getLatestHandoffFile(projectId: string, root: string = getMemoryRoot()): string {
  return path.join(getHandoffsDir(projectId, root), 'latest.json');
}

export function getHandoffHistoryDir(projectId: string, root: string = getMemoryRoot()): string {
  return path.join(getHandoffsDir(projectId, root), 'history');
}

export function getProjectLockFile(projectId: string, root: string = getMemoryRoot()): string {
  return path.join(getProjectDir(projectId, root), '.lock');
}

/** Make a string safe to use as a single path segment. */
export function sanitizeSegment(segment: string): string {
  return segment.replace(/[\\/:*?"<>|\u0000-\u001f]/g, '_').slice(0, 120) || '_';
}
...
export function resolveWorkspacePath(explicit?: string): string {
  const candidate = explicit && explicit.trim() ? explicit.trim() : process.env[ENV_WORKSPACE] || process.cwd();
