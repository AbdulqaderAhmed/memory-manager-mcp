...
describe('git remote normalization', () => {
  ...
  it('strips credentials from URLs', () => {
    expect(normalizeGitRemoteUrl('https://user:token@github.com/company/repo.git')).toBe(
...
describe('project detection', () => {
  ...
  it('detects a git project by remote URL', async () => {
    const repo = await initGitRepo({ remoteUrl: 'https://github.com/company/PMS.git' });
    try {
  ...
  it('same git repository resolves to the same project id from different local paths', async () => {
    ...
    try {
      const [a, b] = await Promise.all([detectProject(repoA.dir), detectProject(repoB.dir)]);
  ...
  it('uses .agent-memory.json identity file when present and no git remote', async () => {
    const ws = await initPlainWorkspace({
      '.agent-memory.json': JSON.stringify({ projectId: 'my-custom-project', memoryVersion: 1 }),
    });
    try {
      const detection = await detectProject(ws.dir);
...
describe('deriveIdentity', () => {
  it('produces stable project ids', () => {
    const a = deriveIdentity({ workspacePath: '/x/y', gitRemoteUrl: 'https://github.com/o/r.git' });
    const b = deriveIdentity({ workspacePath: '/different', gitRemoteUrl: 'git@github.com:o/r.git' });
    expect(projectIdFromCanonical(a.canonical)).toBe(projectIdFromCanonical(b.canonical));
  });
});
