...
describe("memory ranking", () => {
  it("decisions stay relevant much longer than discoveries", () => {
    expect(TYPE_HALF_LIFE_DAYS.decision).toBeGreaterThan(
      TYPE_HALF_LIFE_DAYS.discovery,
    );
  ...
  it("higher importance and confidence rank higher", () => {
    ...
    expect(rankMemory(high, 1, {}, now)).toBeGreaterThan(
      rankMemory(low, 1, {}, now),
    );
  });
  ...
  it("recent memories beat stale ones of the same type", () => {
    ...
    expect(rankMemory(fresh, 1, {}, now)).toBeGreaterThan(
      rankMemory(stale, 1, {}, now),
    );
  });
...
describe("text relevance", () => {
  it("scores phrase matches higher than partial matches", () => {
    const exact = textRelevance(
      "employee permission",
      "Use RBAC for employee permission checks",
    );
    const partial = textRelevance(
      "employee permission",
      "employee scheduling only",
    );
...
describe("MemoryService memory operations", () => {
  ...
  it("saves, retrieves and updates memories", async () => {
    ...
    expect(all.find((m) => m.id === saved.id)?.content).toBe(
      "Use RBAC with fine-grained scopes.",
    );
  });
