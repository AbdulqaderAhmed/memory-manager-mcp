...
describe("CLI", () => {
  ...
  it("projects lists registered projects", async () => {
    const result = await runCli(["projects"], workspace, {
      AGENT_MEMORY_HOME: home,
    });
  ...
  it("project current detects the workspace project", async () => {
    const result = await runCli(["project", "current"], workspace, {
      AGENT_MEMORY_HOME: home,
    });
  ...
  it("project inspect shows counts", async () => {
    const result = await runCli(["project", "inspect"], workspace, {
      AGENT_MEMORY_HOME: home,
    });
  ...
  it("handoff latest prints the handoff", async () => {
    const result = await runCli(["handoff", "latest"], workspace, {
      AGENT_MEMORY_HOME: home,
    });
  ...
  it("sessions lists sessions", async () => {
    const result = await runCli(["sessions"], workspace, {
      AGENT_MEMORY_HOME: home,
    });
  ...
  it("clear requires --all --yes", async () => {
    ...
    const after = await runCli(["projects"], workspace, {
      AGENT_MEMORY_HOME: home,
    });
