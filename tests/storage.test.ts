...
describe('FileSystemMemoryStore', () => {
  ...
  it('creates, reads, updates and deletes projects', async () => {
    ...
    expect(await store.getProject('proj_crud')).toMatchObject({ id: 'proj_crud' });

    project.name = 'renamed';
    await store.saveProject(project);
    expect((await store.getProject('proj_crud'))?.name).toBe('renamed');

    expect((await store.listProjects()).some((p) => p.id === 'proj_crud')).toBe(true);
  ...
  it('saves and retrieves memories, updating by id', async () => {
    ...
    expect((await store.getMemories('proj_mem')).map((m) => m.content)).toEqual(['first']);

    // Update same id -> single memory with new content.
    const m1b = { ...m1, content: 'first-updated', updatedAt: new Date().toISOString() };
  ...
  it('filters memories by type, importance and since', async () => {
    ...
    await store.saveMemory({ ...base, id: 'mem_a', type: 'decision', importance: 0.9 });
    await store.saveMemory({ ...base, id: 'mem_b', type: 'fact', importance: 0.2 });

    const decisions = await store.getMemories('proj_filter', { types: ['decision'] });
    expect(decisions).toHaveLength(1);
    const important = await store.getMemories('proj_filter', { minImportance: 0.5 });
  ...
  it('saves tasks with read-modify-write semantics', async () => {
    ...
    await store.saveTask({
      id: 'task_1', projectId: 'proj_task', title: 'one', status: 'active',
      createdAt: now, updatedAt: now,
    });
    await store.saveTask({
      id: 'task_2', projectId: 'proj_task', title: 'two', status: 'active',
      createdAt: now, updatedAt: now,
    });
    expect(await store.getTasks('proj_task')).toHaveLength(2);

    await store.saveTask({
      id: 'task_1', projectId: 'proj_task', title: 'one-updated', status: 'completed',
      createdAt: now, updatedAt: now,
    });
  ...
  it('saves decisions and sessions', async () => {
    ...
    await store.saveDecision({
      id: 'dec_1', projectId: 'proj_ds', content: 'use RBAC', importance: 0.9,
      confidence: 0.9, status: 'active', createdAt: now, updatedAt: now,
    });
    expect(await store.getDecisions('proj_ds')).toHaveLength(1);

    const session: Session = {
      sessionId: 'sess_1', projectId: 'proj_ds', agentId: 'test',
  ...
  it('saves handoffs to latest and history', async () => {
    ...
    const mk = (id: string, createdAt: string): Handoff => ({
      id, projectId: 'proj_ho', agentId: 'test', task: 'task',
      completed: [], remaining: ['x'], problems: [], changedFiles: [],
      nextAction: 'do it', createdAt,
    });
  ...
  it('clearMemories removes only the memory log', async () => {
    ...
    await store.saveContext({
      projectId: 'proj_clear', name: 'clear', technology: [], status: 'unknown',
